...
def login():
    global lusername
    lusername = input("请输入用户名: ")
    password = input("请输入密码: ")
    md = hashlib.md5(password.encode('utf-8'))
    md=  md.hexdigest()
    request = {"user": lusername, "passwd": md}
    command = 'login\r\n\r\n'
...
if __name__ == "__main__":
        ...
        if action == "login":
            ...
            if k:
                ...
                getFriends(lusername)
                # 创建一个线程用于发送请求x
                getonlinefriends(lusername)
                request_thread = threading.Timer(60,getonlinefriends,args=lusername)
                # 启动请求线程
